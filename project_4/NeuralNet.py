...
class Perceptron(object):
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        """YOUR CODE"""
        inActs = [1] + inActs[:]
        weight = self.getWeightedSum(inActs)
        output = self.sigmoidDeriv(weight)
        return output
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for i in range(self.inSize):
            mod.append(alpha * inputs[i] * delta)
            self.weights[i] = self.weights[i] + mod[i]

        for i in range(len(mod)):
            mod[i] = abs(mod[i])

        totalModification = sum(mod)
