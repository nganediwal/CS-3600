        ...
        for action in [Directions.EAST, Directions.WEST, Directions.NORTH, Directions.SOUTH]:
            ...
            if not hitWall:
                ...
                if (nextx,nexty) in self.corners:
                    for num in range(len(visitedCorners)):
                        if (nextx, nexty) == self.corners[num]:
                            visitedCorners[num] = True
...
def foodHeuristic(state, problem):
    ...
    seperation = 0  #distance between points
    pointX = foodGrid.asList()[0] #start from beginning
    pointY = foodGrid.asList()[0]

    for x in foodGrid.asList():
        for y in foodGrid.asList():
            dist = util.manhattanDistance(x, y) #can't use maze distance
            if (dist > seperation):
                pointX = x
                pointY = y
                seperation = dist #find max separation distance

    xDist = util.manhattanDistance(position, pointX)
    yDist = util.manhattanDistance(position, pointY)

    return min(xDist, yDist) + seperation
