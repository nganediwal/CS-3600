...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for position in self.allPositions:
            obs = self.getObservationProb(observation, pacman_Position, position, jail_Position)
            distance[position] = obs * self.beliefs[position]
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.allPositions:
            nextPosDistance = self.getPositionDistribution(gameState, oldPos) #use getPosition function, pass in gamestate and oldPOs
            oldProb = self.beliefs[oldPos]
            for nextPos in nextPosDistance.keys():
                dist[nextPos] += nextPosDistance[nextPos] * self.beliefs[oldPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for particleNum in range(self.numParticles):
            self.particles.append(self.legalPositions[particleNum%len(self.legalPositions)])
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for pos in self.particles:
            obs = self.getObservationProb(observation, pacman_Position, pos, jail_Position)
            tmp[pos] += obs
        if tmp.total() == 0:
            self.initializeUniformly(gameState)
        else:
    ...
    def elapseTime(self, gameState):
        ...
        for i in range(self.numParticles):
            ...
            if pos in set:
                self.particles[i] = set[pos].sample()
            else:
...
class JointParticleFilter(ParticleFilter):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for prob in self.particles:
            ...
            for x in range(self.numGhosts):
                probability *= self.getObservationProb(observation[x], gameState.getPacmanPosition(), prob[x], self.getJailPosition(x))
            dis[prob] += probability
        self.beliefs = dis

        if self.beliefs.total() != 0:
            self.beliefs.normalize()
            for x in range(self.numParticles):
                sam = self.beliefs.sample()
                self.particles[x] = sam
        else:
            self.initializeUniformly(gameState)
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for x in range(len(newParticle)): 
                newPosDist = self.getPositionDistribution(gameState, newParticle, x, self.ghostAgents[x])
